...
import { Text, StyleSheet, TouchableOpacity, View, Linking, Alert } from 'react-native';
...
export default function CallScreen({ setScreen, screens, roomId }) {
  ...
  const resumeLocalStream = () => {
    ...
    // Send a message to the remote peer to unblur the video
  };
  ...
  const onBackPress = async () => {
    try {
     ...
     /* // Delete offer field from room
      ...
      await callerCandidates?.get()?.then(snapshot => {
  ...
  const startCall = async id => {
    ...
    try {
      ...
      localPC.onicecandidate = e => {
        if (!e.candidate) {
          console.log('got final ice candidate of caller!');
          return;
        }
        console.log('adding caller ice candidate to firestore');
      ...
      localPC.onconnectionstatechange = () => {
        ...
        else if (localPC.connectionState === 'disconnected') {
      ...
      roomRef.onSnapshot(async snapshot => {
        ...
        if (!localPC?.currentRemoteDescription && data?.answer) {
          try {
            ...
            console.log('error onSnapshot =>', err);
          }
        }
      });
        ...
        snapshot.docChanges().forEach(async change => {
          if (change.type === 'added') {
            try {
              ...
              console.log('[Error at Callscreen calleeCandidates', err);
            }
          }
        });
      });
      ...
      <View>
        ...
        {localStream && (
          <View style={styles.callButtons}>
            <TouchableOpacity style={styles.stopCallButton} onPress={onBackPress}>
            ...
            >
              <Text style={styles.buttonText}>{callInProgress ? 'Calling...' : 'Start Call'}</Text>
            </TouchableOpacity>
          </View>
        )}
      </View>
